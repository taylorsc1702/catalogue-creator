...
async function renderHtml(items: Item[], layout: 1 | '1L' | 2 | '2-int' | 3 | 4 | 6 | 8 | 'list' | 'compact-list' | 'table', show: Record<string, boolean>, hyperlinkToggle: 'woodslane' | 'woodslanehealth' | 'woodslaneeducation' | 'woodslanepress', utmParams?: {
  ...
  utmContent?: string;
  utmTerm?: string;
}, itemBarcodeTypes?: {[key: number]: "EAN-13" | "QR Code" | "None"}, barcodeType?: "EAN-13" | "QR Code" | "None", bannerColor?: string, websiteName?: string, coverData?: {
  showFrontCover: boolean;
  showBackCover: boolean;
  frontCoverText1: string;
  frontCoverText2: string;
  ...
  if (coverData) {
    ...
    if (coverData.showFrontCover && coverData.coverImageUrls && coverData.coverImageUrls.length > 0) {
      console.log('Generating front cover HTML with URLs...');
      console.log('Cover image URLs:', coverData.coverImageUrls);
    ...
    if (coverData.showBackCover && coverData.coverImageUrls && coverData.coverImageUrls.length > 0) {
      console.log('Generating back cover HTML with URLs...');
...
<style>
  ...
  * { 
  ...
  .internal-thumbnail-full.image-landscape {
    object-fit: cover;
    width: 160px;
    height: 120px;
    max-width: 160px;
